...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, static_gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "emf.json")):
            scene_info = sceneLoadTypeCallbacks["iphone"](args.source_path)
            dataset_type="iPhone"
        ...
        self.dyn_gaussians._deformation.deformation_net.set_aabb(xyz_max, xyz_min)

        if self.loaded_iter:
            self.dyn_gaussians.load_ply(os.path.join(self.model_path,
                                                           "point_cloud",
                                                           "iteration_" + str(self.loaded_iter),
                                                           "point_cloud.ply"))
            self.dyn_gaussians.load_model(os.path.join(self.model_path,
                                                    "point_cloud",
                                                    "iteration_" + str(self.loaded_iter),
                                                       ))
            self.stat_gaussians.load_ply(os.path.join(self.model_path,
                                                           "point_cloud",
                                                           "iteration_" + str(self.loaded_iter),
    ...
    def save(self, iteration, stage, blcekernel):
        if stage == "coarse":
        ...
        torch.save(blcekernel.model.state_dict(), os.path.join(point_cloud_path, "blce.pth"))

    def save_best_psnr(self, iteration, stage, blcekernel):
        if stage == "coarse":

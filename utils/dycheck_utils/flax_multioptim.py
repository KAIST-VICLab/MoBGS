...
class OptimizerDef:
  ...
  def init_state(self, params):
    ...
    state = OptimizerState(np.asarray(0, dtype=np.int32), param_states)
    return state
...
class Optimizer(struct.PyTreeNode):
  ...
  def compute_gradient(self, loss_fn):
    ...
    def loss_wrapper(target):
      ...
      if isinstance(loss_and_aux, np.ndarray):
...
class MultiOptimizer(OptimizerDef):
  ...
  doing any optimization during the first 1000 steps::
    ...
    new_optimizer = optimizer.apply_gradient(
        ...
        hyper_params=[
          ...
          hparams[1].replace(learning_rate=np.where(step < 1000, 0., lr)),
        ])
  """
  ...
  def init_state(self, params):
    ...
    return OptimizerState(np.asarray(0, dtype=np.int32), param_states)
